...
class HypER(torch.nn.Module):
    def __init__(self, d, d1, d2, **kwargs):
        ...
        self.bn2 = torch.nn.BatchNorm2d(self.in_channels)
        self.bn3 = torch.nn.BatchNorm2d(self.out_channels)
        self.bn4 = torch.nn.BatchNorm1d(d1)
    ...
    def forward(self, e1_idx, r_idx):
        ...
        x = self.bn0(e1)
        x = self.entity_inp_drop(x)

        x = x.permute(1, 0, 2, 3)

        r = self.R(r_idx)

        # relation embedding normalisation
        r = self.bn1(r)
        r = self.relation_inp_drop(r)
